...
#                 if '+' in string_to_check and re.search(r'_\d+\+', string_to_check): #'-' in string_to_check:
...
#                     cp_cnx = '[nc_1]' + '\t' + "\t".join(columns[1:])
#                     cp_cnx_idx = int(cp_cnx.split('\t')[1])
                    
#                     output_lines.append(f"{part1}\t{cp_cnx_idx + 1}\t-\t-\t-\t-\t-\t-\t{cp_cnx_idx}:mod")
#                     output_lines.append(f"{part2}\t{cp_cnx_idx + 2}\t-\t-\t-\t-\t-\t-\t{cp_cnx_idx}:head")
...
def process_file(file_path, output_file_path):
    ...
    for line in lines:
        ...
        if start_checking:
            ...
            if len(columns) > 0:
                string_to_check = columns[0]
                if '+' in string_to_check and re.search(r'_\d+\+', string_to_check):
                    # Split the string and process
                    plus_index = string_to_check.index('+')
                    part1 = string_to_check[:plus_index + 1].strip('+')  # Part before and including '+'
                    part2 = string_to_check[plus_index + 1:]  # Part after '+'
                    ...
                    if len(columns) > 1 and columns[1].strip().isdigit():
                        cp_cnx_idx = int(columns[1].strip())
                        cp_cnx = '[nc_1]' + '\t' + "\t".join(columns[1:])
                        output_lines.append(f"{part1}\t{cp_cnx_idx + 1}\t-\t-\t-\t-\t-\t-\t{cp_cnx_idx}:mod")
                        output_lines.append(f"{part2}\t{cp_cnx_idx + 2}\t-\t-\t-\t-\t-\t-\t{cp_cnx_idx}:head")
                        output_lines.append(cp_cnx)
